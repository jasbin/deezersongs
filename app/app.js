...
io.sockets.on('connection', function (s) {
	...
	request({
		...
		.then(body => {
			...
			if (
				...
				s.emit("message", {
					title: `Version ${lastVersion_MAJOR}.${lastVersion_MINOR}.${lastVersion_PATCH} is available!`,
					msg: body.changelog
				})
			}
		})
	...
	s.on("getChartsTrackListByCountry", function (data) {
		getChartsTrackListByCountry(data.country)
	})
		...
		try {
			...
			if (configFile.userDefined.spotifyUser && spotifySupport) {
				try {
					...
					let offset = 0
					do {
						let data = await Spotify.getUserPlaylists(configFile.userDefined.spotifyUser, {
							fields: "items(images,name,owner.id,tracks.total,uri),total",
							offset: offset * 20
						})
						if (first) {
	...
	s.on("getMyPlaylistList", function (d) {
		getMyPlaylistList()
	})
	...
	s.on("getTrackList", async function (data) {
		...
		if (data.type == 'artist') {
			...
			try {
				...
				let offset = 0
				do {
					let resp = await Spotify.getPlaylistTracks(data.id, {
						fields: "items(track(artists,name,duration_ms,preview_url,explicit)),total",
						offset: offset * 100
					})
					if (first) {
	...
	s.on("saveSettings", function (settings) {
		...
		if (settings.userDefined.chartsCountry != configFile.userDefined.chartsCountry) {
			s.emit("setChartsCountry", {
				selected: settings.userDefined.chartsCountry
			})
	...
	s.on("downloadartist", async data => {
		await downloadArtist(data)
	});
	...
	s.on("downloadartisttop", data => {
		downloadArtistTop(data)
	});
	...
	async function downloadSpotifyPlaylist(data) {
		...
		if (spotifySupport) {
			try {
				...
				data.settings.filename = data.settings.playlistTrackNameTemplate
				data.settings.foldername = data.settings.albumNameTemplate
				var resp = await Spotify.getPlaylist(data.id, {
				...
				do {
					var resp = await Spotify.getPlaylistTracks(data.id, {
						fields: "items(track(artists,name,album,external_ids))",
						offset: offset * 100
					})
			...
			s.emit("message", {
				title: "Spotify Support is not enabled",
				msg: "You should add authCredentials.js in your config files to use this feature<br>You can see how to do that in <a href=\"https://notabug.org/RemixDevs/DeezloaderRemix/wiki/Spotify+Features\">this guide</a>"
			})
		}
	}
	s.on("downloadspotifyplaylist", data => {
		downloadSpotifyPlaylist(data)
	})
	...
	async function convertSpotify2Deezer(track) {
		...
		try {
			...
			logger.err(`Convert2Spotify: ${err.stack ? err.stack : err}`)
		}
	...
	function addToQueue(object) {
		...
		s.downloadQueue[object.queueId].downloadQueuePromise = s.dqueue.push(addNextDownload, {
			args: object
		})
	}
	...
	function cancelDownload(queueId, cleanAll = false) {
		...
		if (cancel) {
			s.emit("cancelDownload", {
				queueId: queueId,
				cleanAll: cleanAll
			});
		}
	}
	s.on("cancelDownload", function (data) {
		cancelDownload(data.queueId)
	});
					...
					downloading.trackList.every(function (t, index) {
						s.trackQueue.push(async cb => {
							...
							if (t.id == 0) {
								...
								t.album = {
									id: 0,
									title: downloading.obj.tracks[t.position].album.name
								}
								t.artist = {
									id: 0,
									name: downloading.obj.tracks[t.position].artists[0].name
								}
							}
							try {
								await downloadTrackObject(t, downloading.queueId, downloading.settings)
	...
	async function downloadTrackObject(track, queueId, settings) {
		...
		if (parseInt(track.id) > 0) {
			...
			if (track.date) {
				...
				switch (settings.dateFormat) {
					case "0":
						date = `${track.date.slicedYear}-${track.date.month}-${track.date.day}`;
						break;
					case "1":
						date = `${track.date.day}-${track.date.month}-${track.date.slicedYear}`;
						break;
					case "2":
						date = `${track.date.month}-${track.date.day}-${track.date.slicedYear}`;
						break;
					case "3":
						date = `${track.date.slicedYear}-${track.date.day}-${track.date.month}`;
						break;
					case "4":
						date = `${track.date.day}${track.date.month}`;
						break;
					default:
						date = `${track.date.day}${track.date.month}`;
						break;
		...
		if (track.album.pictureUrl) {
			...
			if (fs.existsSync(imgPath)) {
				...
				try {
					var body = await request.get(track.album.pictureUrl, {
						strictSSL: false,
						encoding: 'binary'
					})
		...
		if (parseInt(this.id) > 0 && track.album.artist.picture && settings.saveArtworkArtist) {
			...
			if (settings.createArtistFolder) {
				...
				if (!fs.existsSync(imgPath)) {
					try {
						var body = await request.get(track.album.artist.pictureUrl, {
							strictSSL: false,
							encoding: 'binary'
						})
		...
		var downloadingPromise = new Promise((resolve, reject) => {
			let req = requestOld.get({
				url: track.getDownloadUrl(track.selectedFormat),
				strictSSL: false,
